...
<body>
	...
	<form action="clan.php" method="GET">
		<input type="text" id="tag-clan" name="tag" placeholder="TAG, ex: VASS" />
		<select name="tier">
	...
	foreach($clanSkirmishData as $playerName => $playerData) {
		...
		if( sizeof($tankData) > 0 ) {
			...
			for($i = 0; $i < min(sizeof($sortedTankData), 8) ; $i++) {
				...
				printf(" %s ", $tank["name"]);
				printf(" <i> %3d%% </i> ", 100 * $tank["battles_count"] / $playerSkirmishCount);
				echo "</td>";
			}
			echo "</tr>";
		}
...
if( isset($_GET['tag']) and isset($_GET['tier']) ) {
	render(getClanSkirmishData($_GET['tag'], intval($_GET['tier'])));
}
?>
